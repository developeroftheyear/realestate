...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('panel.agents.create') }}" class="bg-indigo-600 text-white px-4 py-2 rounded shadow hover:bg-indigo-700 transition">Add New Agent</a>
</div>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <a href="{{ route('panel.agents.edit', $agent->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                    <form action="{{ route('panel.agents.destroy', $agent->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                        @csrf
                        @method('DELETE')
